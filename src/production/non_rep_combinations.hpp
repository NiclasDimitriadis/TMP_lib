...
private:
   ...
   template<size_t cohort_len_, size_t I>
   struct calc_index {
      // describes value in first column in relation to n, r and cohort length
      static constexpr size_t value = I + 2 - zero_indexed + (n - r) - cohort_len_;
   };
   ...
   struct compute {
      using type = compute<cohort_len, cohort_len_rem - 1, comb_Is...,
         ...
         calc_index<cohort_len, r - 1>::value + cohort_len - cohort_len_rem>::type;
   };

   // last combination in cohort
   template<size_t cohort_len, size_t... comb_Is>
   struct compute<cohort_len, 1, comb_Is...> {
      using type = compute<cohort_len - 1, cohort_len - 1, comb_Is...,
         calc_index<cohort_len, Is>::value...,
         calc_index<cohort_len, r - 1>::value + cohort_len - 1>::type;
   };

   // final combination
   template<size_t... comb_Is>
   struct compute<1, 1, comb_Is...> {
      using type = std::integer_sequence<size_t, comb_Is..., calc_index<1, Is>::value...,
