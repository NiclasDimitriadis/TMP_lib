...
template<template<typename...> class Monoid_Templ, size_t order,
   typename... Ts>
struct monoid_check
    : monoid_check<Monoid_Templ, order, param_pack::type_pack_t<Ts...>> {};

template<template<typename...> class Monoid_Templ, size_t order,
   typename Type_Pack>
requires param_pack::type_pack_convertible_v<Type_Pack> && monoidal_class_template::check_closure_property_v<Monoid_Templ, order, param_pack::generate_type_pack_t<Type_Pack>>
struct monoid_check<Monoid_Templ, order, Type_Pack> {
private:
   using type_pack = param_pack::generate_type_pack_t<Type_Pack>;
   // apply Monoid_Templ to all non repeating combiantions of types of length
   // order
