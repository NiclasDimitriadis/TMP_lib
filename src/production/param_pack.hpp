...
// type
template <auto...> struct single_type_nt_pack : std::false_type {};
...
template <auto... is> using pack_type_t = typename pack_type<is...>::type;

template <typename T, T... vals> struct non_type_pack {
private:
  // abridged version of type_pack_t internal to non_type_pack, avoids circular
  // dependencies
  template <typename... Ts> struct type_pack_pr {
  private:
    template <typename> struct append_logic {
      static_assert(false);
    };

    template <typename... Us> struct append_logic<type_pack_pr<Us...>> {
  ...
  template <typename, auto...> struct append_logic {
    static_assert(false);
  };

  template <T... app_vals> struct append_logic<non_type_pack<T, app_vals...>> {
  ...
  template <size_t index, auto... vals_> struct reverse_logic {
    using type =
        non_type_pack<T, pack_index_v<index, vals_...>>::template append_t<
  ...
  template <auto... vals_> struct reverse_logic<0, vals_...> {
    using type = non_type_pack<T, pack_index_v<0, vals_...>>;
  };

  template <auto...> struct functor_map_logic {
    static_assert(false);
  };
  ...
  template <auto callable> struct functor_map_logic<callable> {
    using type = non_type_pack<decltype(callable(std::declval<T>()))>;
  };

  template <auto callable>
    requires std::is_invocable_v<decltype(callable), T> &&
             (!std::is_same_v<decltype(callable(std::declval<T>())), void>)
  struct functor_map {
  ...
  struct same_type_and_size {
    static constexpr bool value = Other_NTP::size == sizeof...(vals) &&
                                  std::is_same_v<T, typename Other_NTP::type>;
  };

  template <size_t index, size_t iter, typename TPack, T... elements>
  struct extract_nth_element {
    static constexpr T element =
        TPack::template index_t<index>::template index_v<iter>;
    using type =
        extract_nth_element<index + 1, iter, TPack, elements..., element>::type;
  };
  ...
  template <template <T...> class Class_Template, size_t index, typename TPack,
            auto... results>
  struct pure_logic {
    using iter_pack = extract_nth_element<0, index, TPack>::type;
    static constexpr auto result =
        iter_pack::template specialize_template_t<Class_Template>::value;
    using type =
        pure_logic<Class_Template, index + 1, TPack, results..., result>::type;
  };

  template <template <T...> class Class_Template, typename TPack,
            auto... results>
  ...
  functor_map_t<same_type_and_size>::template fold_t<
      helpers::And, std::true_type>::value struct applicative_pure {
    using type = non_type_pack<decltype(Class_Template<>::value),
                               Class_Template<>::value>;
  };

  template <template <T...> class Class_Template, typename Other_Packs, T fst,
            T... tail>
    requires Other_Packs::template
  functor_map_t<same_type_and_size>::template fold_t<helpers::And,
                                                     std::true_type>::
      value struct applicative_pure<Class_Template, Other_Packs, fst, tail...> {
    using TPack = type_pack_pr<
        non_type_pack<T, fst, tail...>>::template append_t<Other_Packs>;
    using type = pure_logic<Class_Template, 0, TPack>::type;
  };

  template <template <auto...> class, auto...> struct monadic_bind_logic {
    using type = non_type_pack<T>;
  };
  ...
  template <template <auto...> class Class_Template> struct monadic_bind {
  ...
  struct fold_logic {
    static constexpr T value =
        fold_logic<fold_by, fold_by(first, second), tail...>::value;
  };
...
public:
  ...
  template <size_t n> using head_t = truncate_back_t<sizeof...(vals) - n>;

  // yields last n elements
  template <size_t n> using tail_t = truncate_front_t<sizeof...(vals) - n>;

  // extract i-th entry in parameter pack
  template <size_t i> static constexpr T index_v = pack_index_v<i, vals...>;
  ...
  template <auto callable> using functor_map_t = functor_map<callable>::type;

  // apply a class template to multiple non_type_pack-types as an applicative
  // functor
  template <template <type...> class Class_Template,
            typename... Other_NTP_Packs>
  using applicative_pure_t =
      applicative_pure<Class_Template, type_pack_pr<Other_NTP_Packs...>,
                       vals...>::type;
...
template <typename> struct generate_non_type_pack {};
...
constexpr inline bool non_type_pack_convertible_v =
    non_type_pack_convertible<T>::value;
...
template <typename T> using generate_type_pack_t = generate_type_pack<T>::type;
